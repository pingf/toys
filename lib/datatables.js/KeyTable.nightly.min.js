...
}};this.fnBlur=function(){u()};var f=null;var m=null;var c=null;var a=null;var G=null;
var i="focus";var w=false;var r={action:[],esc:[],focus:[],blur:[]};var e=null;var n;
var s;var C=false;function B(H){return function(I,L,K){if((I===null||typeof I=="number")&&(L===null||typeof L=="number")&&typeof K=="function"){x(H,I,L,K)
...
w=Q}var N=j(M);m=M;c=N[0];a=N[1];var R,T,L,O,H,I,S;if(U){R=$(window).height();T=$(window).width();
L=$(document).scrollTop();O=$(document).scrollLeft();H=M.offsetHeight;I=M.offsetWidth;
S=y(M);if(e&&typeof J.oScroll!="undefined"&&(J.oScroll.sX!==""||J.oScroll.sY!=="")){S[1]-=$(J.nTable.parentNode).scrollTop();
...
}else{b(E(H.focus[0],H.focus[1]),H.initScroll)}D()}if(jQuery.browser&&(jQuery.browser.mozilla||jQuery.browser.opera)){jQuery(document).bind("keypress",h)
}else{jQuery(document).bind("keydown",h)}if(e){jQuery(e.fnSettings().nTable).on("click","td",l)
}else{jQuery(f).on("click","td",l)}jQuery(document).click(function(L){var M=L.target;
var K=false;while(M){if(M==H.table){K=true;break}M=M.parentNode}if(!K){u()}})}v(q,this)
}KeyTable.VERSION="1.1.8.dev";KeyTable.prototype.VERSION=KeyTable.VERSION;